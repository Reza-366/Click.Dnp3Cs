...
public sealed class LoggingHandler : IAPDUHandler
{
	...
	private static string ToUTCString(in DNPTime dnptime)
	{
		...
		var milliseconds = (UInt16)(dnptime.value % 1000);
	...
	private static string ToHex(System.Byte b)
	{
		std::ostringstream oss = new std::ostringstream();
...
//ORIGINAL LINE: auto logItem = [this](const Indexed<ControlRelayOutputBlock>& item)
		...
		{
			std::ostringstream oss = new std::ostringstream();
			oss << "[" << item.index << "] - code: 0x" << ToHex(new System.Byte(item.value.rawCode)) << " (" << "op type: " << OperationTypeSpec.to_human_string(item.value.opType) << ", tcc: " << TripCloseCodeSpec.to_human_string(item.value.tcc) << ", cr: " << item.value.clear << ")";
			oss << " count: " << (size_t)item.value.count;
			oss << " on-time: " << (size_t)item.value.onTimeMS;
			oss << " off-time: " << (size_t)item.value.offTimeMS;
			oss << " status: " << CommandStatusSpec.to_human_string(item.value.status);
			if (logger.is_enabled(opendnp3.flags.Globals.APP_OBJECT_RX))
			{
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
...
//ORIGINAL LINE: return IINField::Empty();
	...
	private override bool IsAllowed(System.UInt32 UnnamedParameter, GroupVariation UnnamedParameter2, QualifierCode UnnamedParameter3)
	{
		return true;
	}
