...
//ORIGINAL LINE: template<class T>
	public void SelectAndOperate<T>(in T command, UInt16 index, in CommandResultCallbackT callback, in TaskConfig config = TaskConfig.Default())
	{
		CommandSet commands = new CommandSet({opendnp3.Globals.WithIndex(command, new UInt16(index))});
...
//C++ TO C# CONVERTER WARNING: The original C++ template specifier was replaced with a C# generic specifier, which may not produce the same behavior:
//ORIGINAL LINE: template<class T>
	public void DirectOperate<T>(in T command, UInt16 index, in CommandResultCallbackT callback, in TaskConfig config = TaskConfig.Default())
	{
		CommandSet commands = new CommandSet({opendnp3.Globals.WithIndex(command, new UInt16(index))});
