...
public sealed class TypedCommandHeader <T> : ICommandHeader, ICommandCollection<T>
{
	...
	public override ICommandCollection<T> Add(in T command, UInt16 index)
	{
		if (index > System.Byte.MaxValue)
		{
			this.use_single_byte_index = false;
		}

		this.records.Add(opendnp3.Globals.WithIndex(command, new UInt16(index)));
		return this;
	}
...
//ORIGINAL LINE: virtual bool AreAllSelected() const override
	...
	public override bool Write(HeaderWriter writer, IndexQualifierMode mode)
	{
		...
		if (this.use_single_byte_index)
		{
			...
			{
				if (!iter.Write(rec.command, (System.Byte)rec.index))
				{
					return false;
				}
			}
		...
		{
			var iter = writer.IterateOverCountWithPrefix<ser4cpp.Globals.Bit16<UInt16, 0, 1>, T>(QualifierCode.UINT16_CNT_UINT16_INDEX, this.serializer);

			foreach (var rec in this.records)
			{
				if (!iter.Write(rec.command, new UInt16(rec.index)))
				{
					return false;
				}
			}
