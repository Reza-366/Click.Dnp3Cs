 ...
 */
public sealed class EventScanTask : PollTaskBase
{

	public EventScanTask(TaskContext context, IMasterApplication application, ISOEHandler soeHandler, ClassField classes, in Logger logger) : base(context, application, std::move(soeHandler), TaskBehavior.ReactsToIINOnly(), logger, TaskConfig.Default())
	{
		this.classes = new opendnp3.ClassField(classes.OnlyEventClasses());
	}
...
//ORIGINAL LINE: virtual int Priority() const override
	...
	{
		return opendnp3.priority.Globals.EVENT_SCAN;
	}
...
//ORIGINAL LINE: virtual bool BlocksLowerPriority() const override
	...
	private readonly ClassField classes = new ClassField();
