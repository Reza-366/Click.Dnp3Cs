...
public abstract class IMasterOperations : ICommandProcessor, System.IDisposable
{
	 ...
	 * @return A proxy class used to manipulate the scan
	 */
	public abstract IMasterScan AddRangeScan(GroupVariationID gvId, UInt16 start, UInt16 stop, TimeDuration period, ISOEHandler soe_handler, in TaskConfig config = TaskConfig.Default());
	 ...
	 */
	public abstract void ScanRange(GroupVariationID gvId, UInt16 start, UInt16 stop, ISOEHandler soe_handler, in TaskConfig config = TaskConfig.Default());

	/**
	 * Write a time and interval object to a specific index
	 */
	public abstract void Write(in TimeAndInterval value, UInt16 index, in TaskConfig config = TaskConfig.Default());
