...
//	DefaultListenCallbacks();
	...
	public override bool AcceptConnection(uint64_t sessionid, in string ipaddress)
	{
		return true;
	}

	public override bool AcceptCertificate(uint64_t sessionid, in X509Info info)
	{
		return true;
	}
...
//ORIGINAL LINE: return TimeDuration::Seconds(30);
	...
	public override void OnFirstFrame(uint64_t sessionid, in LinkHeaderFields header, ISessionAcceptor acceptor)
	{
...
//ORIGINAL LINE: auto master = acceptor.AcceptSession(SessionIdToString(sessionid), soe, app, config);
		var master = acceptor.AcceptSession(SessionIdToString(new uint64_t(sessionid)), new opendnp3.PrintingSOEHandler(soe), new opendnp3.DefaultMasterApplication(app), config);
	}

	public override void OnConnectionClose(uint64_t sessionid, IMasterSession session)
	{
		// full implementations would drop any references they're holding to this session
		// shared_ptr can be used with == operator also
	}

	public override void OnCertificateError(uint64_t sessionid, in X509Info info, int error)
	{
	}

	private string SessionIdToString(uint64_t id)
	{
