...
//ORIGINAL LINE: return Parse(buffer, handler, &logger, settings);
	...
	public static ParseResult ParseSinglePass(in ser4cpp.rseq_t buffer, Logger pLogger, IAPDUHandler pHandler, IWhiteList pWhiteList, in ParserSettings settings)
	{
		...
		while (copy.length() > 0)
		{
			var result = ParseHeader(copy, pLogger, new System.UInt32(count), settings, pHandler, pWhiteList);
			++count;
	...
	private static bool AllowAll(System.UInt32 headerCount, GroupVariation gv, QualifierCode qc)
	{
		return true;
	}
...
//	static ParseResult ParseHeaders(in ser4cpp::rseq_t buffer, Logger pLogger, in ParserSettings settings, IAPDUHandler pHandler);
	...
	private static ParseResult ParseHeader(ser4cpp.rseq_t buffer, Logger pLogger, System.UInt32 count, in ParserSettings settings, IAPDUHandler pHandler, IWhiteList pWhiteList)
	{
		...
		var GV = GroupVariationRecord.GetRecord(new System.Byte(header.group), new System.Byte(header.variation));
	...
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
		...
		if (pWhiteList != null && !pWhiteList.IsAllowed(new System.UInt32(count), GV.enumeration, QualifierCodeSpec.from_type(new System.Byte(header.qualifier))))
		{
			if (pLogger != null && pLogger.is_enabled(opendnp3.flags.Globals.WARN))
			{
				string message_buffer_some_name_no_conflict = new string(new char[opendnp3.Globals.max_log_entry_size]);
...
//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	static ParseResult ParseCountOfIndices(ser4cpp::rseq_t buffer, in HeaderRecord record, in NumParser numparser, UInt16 count, Logger pLogger, IAPDUHandler pHandler);
}
