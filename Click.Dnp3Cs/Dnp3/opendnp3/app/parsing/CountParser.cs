...
{
	private delegate void HandleFun(in HeaderRecord record, UInt16 count, in ser4cpp.rseq_t buffer, IAPDUHandler handler);
	...
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
			...
			{
				return ParseCountOfObjects(buffer, record, new UInt16(count), pLogger, pHandler);
			}
...
//ORIGINAL LINE: template<class Descriptor>
	private static CountParser From<Descriptor>(UInt16 count)
	{
		var size = (size_t)count * Descriptor.Size();
		return new CountParser(count, size, InvokeCountOf<Descriptor>);
	}

	private static ParseResult ParseCountOfObjects(ser4cpp.rseq_t buffer, in HeaderRecord record, UInt16 count, Logger pLogger, IAPDUHandler pHandler)
	{
...
//C++ TO C# CONVERTER WARNING: The original C++ template specifier was replaced with a C# generic specifier, which may not produce the same behavior:
//ORIGINAL LINE: template<class Descriptor>
	private static void InvokeCountOf<Descriptor>(in HeaderRecord record, UInt16 count, in ser4cpp.rseq_t buffer, IAPDUHandler handler)
	{
//C++ TO C# CONVERTER TASK: Lambda expressions cannot be assigned to 'var':
		...
		var collection = opendnp3.Globals.CreateBufferedCollection<T>(buffer, new UInt16(count), read);
		handler.OnHeader(new CountHeader(record, count), collection);
	}

	private CountParser(UInt16 count, size_t required_size, HandleFun handler)
	{
...
//ORIGINAL LINE: this.required_size = required_size;
	...
	private size_t required_size = new size_t();
