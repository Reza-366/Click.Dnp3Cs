...
//ORIGINAL LINE: template<class Type>
	...
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
			...
			if (pHandler != null)
			{
				...
				var collection = opendnp3.Globals.CreateBufferedCollection<new Indexed<OctetString>>(buffer, new size_t(COUNT), read);
...
//ORIGINAL LINE: auto read = [range](ser4cpp::rseq_t& buffer, System.UInt32 pos)
		var read = (ser4cpp.rseq_t buffer, System.UInt32 pos) =>
		{
		...
		var collection = opendnp3.Globals.CreateBufferedCollection<new Indexed<typename Descriptor.Target>>(buffer, new size_t(COUNT), read);
...
//ORIGINAL LINE: template<class Type>
	private static void InvokeRangeOfType<Type>(in HeaderRecord record, in Range range, in ser4cpp.rseq_t buffer, IAPDUHandler handler)
	{
		...
		var collection = opendnp3.Globals.CreateBufferedCollection<new Indexed<Type>>(buffer, new size_t(COUNT), read);
...
//ORIGINAL LINE: template<class Type>
	private static void InvokeRangeDoubleBitfieldType<Type>(in HeaderRecord record, in Range range, in ser4cpp.rseq_t buffer, IAPDUHandler handler)
	{
		...
		var read = (ser4cpp.rseq_t buffer, size_t pos) =>
		{
			Type value = new Type(GetDoubleBit(buffer, pos));
			return new opendnp3.Indexed<Type>(opendnp3.Globals.WithIndex(value, (UInt16)(range.start + pos)));
		});
	...
	private RangeParser(in Range range, size_t requiredSize, HandleFun handler)
	{
