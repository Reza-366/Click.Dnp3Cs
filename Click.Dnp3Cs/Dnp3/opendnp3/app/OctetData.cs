...
{
	public const System.Byte MAX_SIZE = 255;
	...
	{
		this.size = input.length == 0 ? 1 : ser4cpp.Globals.min<System.Byte>(new System.Byte(MAX_SIZE), (System.Byte)input.length);
		RSeq<size_t> input_slice = new RSeq<size_t>(input.data, input.length);
		if (input_slice.is_not_empty())
		{
			WSeq<size_t> dest = new WSeq<size_t>(buffer.data(), buffer.Count);
			dest.copy_from(input_slice.take(size));
		}
	}

//C++ TO C# CONVERTER WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: inline System.Byte Size() const
	...
	public bool Set(in Buffer input)
	{
		...
		System.Byte usable_size = is_oversized ? MAX_SIZE : (System.Byte)input_slice.length();
...
//C++ TO C# CONVERTER TASK: There is no equivalent to 'reinterpret_cast' in C#:
		return this.Set(new Buffer(reinterpret_cast<const System.Byte>(input), length > MAX_SIZE ? MAX_SIZE : length));
	}
...
//C++ TO C# CONVERTER TASK: There is no equivalent to 'reinterpret_cast' in C#:
		return new Buffer(reinterpret_cast<const System.Byte>(input), length > MAX_SIZE ? MAX_SIZE : length);
	}
