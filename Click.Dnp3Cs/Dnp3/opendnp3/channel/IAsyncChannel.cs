...
public abstract class IAsyncChannel : Uncopyable
{
	...
	public bool BeginRead(in WSeq<size_t> buffer)
	{
		Debug.Assert(callbacks);
		if (this.CanRead())
		{
			this.reading = true;
			this.BeginReadImpl(new WSeq<size_t>(buffer));
			return true;
		}
		else
		{
...
//ORIGINAL LINE: inline bool CanWrite() const
	...
	protected void OnReadCallback(in std::error_code ec, size_t num)
	{
		this.reading = false;
		if (this.callbacks != null && !is_shutting_down)
		{
			this.callbacks.OnReadComplete(ec, new size_t(num));
		}
	}

	protected void OnWriteCallback(in std::error_code ec, size_t num)
	{
		this.writing = false;
		if (this.callbacks != null && !is_shutting_down)
		{
			this.callbacks.OnWriteComplete(ec, new size_t(num));
		}
	}
...
//ORIGINAL LINE: auto action = [self]()
	...
	private abstract void BeginReadImpl(WSeq<size_t> buffer);
