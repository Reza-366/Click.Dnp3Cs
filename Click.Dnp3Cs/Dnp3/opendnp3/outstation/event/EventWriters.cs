...
//ORIGINAL LINE: template<class T>
	public static UInt16 Write<T>(HeaderWriter writer, IEventCollection<T> items, in DNP3Serializer<T> serializer)
	{
		BasicEventWriter<T> handler = new BasicEventWriter<T>(writer, serializer);
		return new UInt16(items.WriteSome(handler));
	}

//C++ TO C# CONVERTER WARNING: The original C++ template specifier was replaced with a C# generic specifier, which may not produce the same behavior:
//ORIGINAL LINE: template<class T>
	public static UInt16 WriteWithCTO<T>(in DNPTime cto, HeaderWriter writer, IEventCollection<T> items, in DNP3Serializer<T> serializer)
	{
...
//ORIGINAL LINE: value.time = cto;
	...
	public static UInt16 Write(System.Byte firstSize, HeaderWriter writer, IEventCollection<OctetString> items)
	{
...
//ORIGINAL LINE: template<class T>
	private sealed class BasicEventWriter <T> : IEventWriter<T>
	{
		...
		public override bool Write(in T meas, UInt16 index)
		{
			return iterator.IsValid() ? iterator.Write(meas, new UInt16(index)) : false;
		}
	}
...
//ORIGINAL LINE: template<class T, class U>
	private sealed class CTOEventWriter <T, U> : IEventWriter<T>
	{
		...
		public override bool Write(in T meas, UInt16 index)
		{
			...
			// can't encode timestamps where the diff is greater than UInt16
			...
			return this.iterator.Write(copy, new UInt16(index));
		}
	}
}
...
public class OctetStringEventWriter : IEventWriter<OctetString>
{
	...
	public OctetStringEventWriter(HeaderWriter writer, System.Byte size)
	{
	...
	public override bool Write(in OctetString meas, UInt16 index)
	{
		...
		return iterator.Write(meas, new UInt16(index));
	}
}
