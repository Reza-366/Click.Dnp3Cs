...
//	OutstationState ContinueMultiFragResponse(in Addresses addresses, in AppSeqNum seq);
	...
	private OutstationState RespondToReadRequest(in ParsedRequest request)
	{
		...
		return this.BeginResponseTx(new UInt16(request.addresses.source), response);
	}
...
//ORIGINAL LINE: opendnp3::HexLogging::log(this->logger, flags::APP_HEX_RX, message.payload, ' ', 18, 18);
		...
		{
			UpdateLastBroadcastMessageReceived(new UInt16(message.addresses.destination));
		}
...
//C++ TO C# CONVERTER TASK: C# does not allow fall-through from a non-empty 'case':
	...
	private OutstationState BeginResponseTx(UInt16 destination, APDUResponse response)
	{
		...
		this.BeginTx(new UInt16(destination), data);
	...
	private void BeginRetransmitLastResponse(UInt16 destination)
	{
		this.BeginTx(new UInt16(destination), this.sol.tx.GetLastResponse());
	}

	private void BeginRetransmitLastUnsolicitedResponse()
	{
		this.BeginTx(new UInt16(this.addresses.destination), this.unsol.tx.GetLastResponse());
	}
	...
	private void BeginUnsolTx(APDUResponse response)
	{
		...
		this.BeginTx(new UInt16(this.addresses.destination), data);
	}

//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	void BeginTx(UInt16 destination, in RSeq<size_t> message);
...
//ORIGINAL LINE: return ret;
	...
	private void UpdateLastBroadcastMessageReceived(UInt16 destination)
	{
		switch (destination)
		{
