...
namespace opendnp3
{
	...
	//enum class LinkTransmitMode : System.Byte
	//{
	//    Idle,
	//    Primary,
	//    Secondary
...
//	ser4cpp::rseq_t FormatPrimaryBufferWithUnconfirmed(in Addresses addr, in ser4cpp::rseq_t tpdu);
	...
	public void QueueAck(UInt16 destination)
	{
		var dest = secTxBuffer.as_wseq();
		var buffer = LinkFrame.FormatAck(dest, config.IsMaster, false, new UInt16(destination), new UInt16(this.config.LocalAddr), logger);
...
//ORIGINAL LINE: opendnp3::HexLogging::log(logger, flags::LINK_TX_HEX, buffer, ' ', 10, 18);
	...
	public void QueueLinkStatus(UInt16 destination)
	{
		var dest = secTxBuffer.as_wseq();
		var buffer = LinkFrame.FormatLinkStatus(dest, config.IsMaster, false, new UInt16(destination), new UInt16(this.config.LocalAddr), logger);
		if (logger.is_enabled(opendnp3.flags.Globals.LINK_TX_HEX))
		{
//C++ TO C# CONVERTER TASK: The following line was determined to contain a copy constructor call - this should be verified and a copy constructor should be created:
//ORIGINAL LINE: opendnp3::HexLogging::log(logger, flags::LINK_TX_HEX, buffer, ' ', 10, 18);
	...
	public void QueueRequestLinkStatus(UInt16 destination)
	{
		var dest = priTxBuffer.as_wseq();
		var buffer = LinkFrame.FormatRequestLinkStatus(dest, config.IsMaster, new UInt16(destination), new UInt16(this.config.LocalAddr), logger);
		if (logger.is_enabled(opendnp3.flags.Globals.LINK_TX_HEX))
		{
//C++ TO C# CONVERTER TASK: The following line was determined to contain a copy constructor call - this should be verified and a copy constructor should be created:
//ORIGINAL LINE: opendnp3::HexLogging::log(logger, flags::LINK_TX_HEX, buffer, ' ', 10, 18);
