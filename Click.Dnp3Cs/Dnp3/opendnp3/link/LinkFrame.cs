...
//	static RSeq<size_t> FormatAck(WSeq<size_t> buffer, bool aIsMaster, bool aIsRcvBuffFull, UInt16 aDest, UInt16 aSrc, Logger pLogger);
//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	static RSeq<size_t> FormatNack(WSeq<size_t> buffer, bool aIsMaster, bool aIsRcvBuffFull, UInt16 aDest, UInt16 aSrc, Logger pLogger);
//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	static RSeq<size_t> FormatLinkStatus(WSeq<size_t> buffer, bool aIsMaster, bool aIsRcvBuffFull, UInt16 aDest, UInt16 aSrc, Logger pLogger);
//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	static RSeq<size_t> FormatNotSupported(WSeq<size_t> buffer, bool aIsMaster, bool aIsRcvBuffFull, UInt16 aDest, UInt16 aSrc, Logger pLogger);
...
//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	static RSeq<size_t> FormatTestLinkStatus(WSeq<size_t> buffer, bool aIsMaster, bool aFcb, UInt16 aDest, UInt16 aSrc, Logger pLogger);
//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	static RSeq<size_t> FormatResetLinkStates(WSeq<size_t> buffer, bool aIsMaster, UInt16 aDest, UInt16 aSrc, Logger pLogger);
//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	static RSeq<size_t> FormatRequestLinkStatus(WSeq<size_t> buffer, bool aIsMaster, UInt16 aDest, UInt16 aSrc, Logger pLogger);
//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	static RSeq<size_t> FormatConfirmedUserData(WSeq<size_t> buffer, bool aIsMaster, bool aFcb, UInt16 aDest, UInt16 aSrc, RSeq<size_t> user_data, Logger pLogger);
//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	static RSeq<size_t> FormatUnconfirmedUserData(WSeq<size_t> buffer, bool aIsMaster, UInt16 aDest, UInt16 aSrc, RSeq<size_t> user_data, Logger pLogger);
	...
	*/
	public static void ReadUserData(System.Byte pSrc, System.Byte pDest, size_t len)
	{
		size_t length = new size_t(len);
//C++ TO C# CONVERTER TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
		System.Byte * pRead = new System.Byte(pSrc);
//C++ TO C# CONVERTER TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
		System.Byte * pWrite = new System.Byte(pDest);

		while (length > 0)
		{
			size_t max = opendnp3.Globals.LPDU_DATA_BLOCK_SIZE;
			size_t num = (length <= ser4cpp.Globals.max) ? length : ser4cpp.Globals.max;
...
//C++ TO C# CONVERTER TASK: Pointer arithmetic is detected on the parameter 'pBody', so pointers on this parameter are left unchanged:
	public static bool ValidateBodyCRC(in System.Byte * pBody, size_t length)
	{
		while (length > 0)
		{
			size_t max = opendnp3.Globals.LPDU_DATA_BLOCK_SIZE;
			...
			if (CRC.IsCorrectCRC(new System.Byte(pBody), new size_t(num)))
			{
				pBody += (num + 2);
				length -= num;
			}
	...
	public static size_t CalcFrameSize(size_t dataLength)
	{
		return opendnp3.Globals.LPDU_HEADER_SIZE + CalcUserDataSize(new size_t(dataLength));
	}

	private static size_t CalcUserDataSize(size_t dataLength)
	{
		if (dataLength > 0)
		{
			size_t mod16 = dataLength % opendnp3.Globals.LPDU_DATA_BLOCK_SIZE;
			size_t size = (dataLength / opendnp3.Globals.LPDU_DATA_BLOCK_SIZE) * opendnp3.Globals.LPDU_DATA_PLUS_CRC_SIZE; // complete blocks
...
//C++ TO C# CONVERTER TASK: Pointer arithmetic is detected on the parameter 'pDest', so pointers on this parameter are left unchanged:
	private static void WriteUserData(in System.Byte * pSrc, System.Byte * pDest, size_t length)
	{
		while (length > 0)
		{
			System.Byte max = opendnp3.Globals.LPDU_DATA_BLOCK_SIZE;
			size_t num = length > ser4cpp.Globals.max ? ser4cpp.Globals.max : length;
//C++ TO C# CONVERTER TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(pDest, pSrc, num);
			CRC.AddCrc(new System.Byte(pDest), new size_t(num));
			pSrc += num;
			pDest += (num + 2);
			length -= num;
		}
...
//C++ TO C# CONVERTER TASK: The implementation of the following method could not be found:
//	static RSeq<size_t> FormatHeader(WSeq<size_t> buffer, System.Byte aDataLength, bool aIsMaster, bool aFcb, bool aFcvDfc, LinkFunction aFuncCode, UInt16 aDest, UInt16 aSrc, Logger pLogger);
}
