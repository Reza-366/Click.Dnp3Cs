...
public class CRC
{
	public static UInt16 CalcCrc(System.Byte[] input, size_t length)
	{
		UInt16 CRC = 0;

		for (System.UInt32 i = 0; i < length; ++i)
		{
			System.Byte index = (CRC ^ input[i]) & 0xFF;
//C++ TO C# CONVERTER TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//ORIGINAL LINE: CRC = crcTable[index] ^ (CRC >> 8);
			CRC.CopyFrom(crcTable[index] ^ (CRC >> 8));
		}
...
//	static UInt16 CalcCrc(in RSeq<size_t> view);

	public static void AddCrc(System.Byte input, size_t length)
	{
		UInt16 crc = CRC.CalcCrc(input, new size_t(length));

		ser4cpp.wseq_t buffer = new ser4cpp.wseq_t(input + length, 2);
		ser4cpp.Globals.Bit16<UInt16, 0, 1>.write_to(buffer, crc);
	}

	public static bool IsCorrectCRC(System.Byte input, size_t length)
	{
