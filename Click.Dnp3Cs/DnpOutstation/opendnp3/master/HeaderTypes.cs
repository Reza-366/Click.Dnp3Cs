...
//union HeaderUnion
//{
//	StartStopRange<byte> range8;
//	StartStopRange<ushort> range16;
//	Count<byte> count8;
//	Count<ushort> count16;
//};
...
//ORIGINAL LINE: return writer.WriteHeader(id, QualifierCode::ALL_OBJECTS);
		...
		case (HeaderType.Ranged16):
			return writer.WriteRangeHeader<new ser4cpp.Globals.Bit16<ushort, 0, 1>>(QualifierCode.UINT16_START_STOP, id, value.range16.start, value.range16.stop);
		...
		case (HeaderType.LimitedCount16):
			return writer.WriteCountHeader<new ser4cpp.Globals.Bit16<ushort, 0, 1>>(QualifierCode.UINT16_CNT, id, value.count16.value);
		default:
			return false;
		}
	}
...
//ORIGINAL LINE: return Header::AllObjects(60, 4);
	 ...
	 */
	public static Header Range8(byte group, byte variation, byte start, byte stop)
	{
	 ...
	 */
	public static Header Range16(byte group, byte variation, ushort start, ushort stop)
	{
		return new Header(group, variation, start, stop);
	}
	 ...
	 */
	public static Header Count8(byte group, byte variation, byte count)
	{
	 ...
	 */
	public static Header Count16(byte group, byte variation, ushort count)
	{
		return new Header(group, variation, count);
	}
...
//	Header() = default;
	...
	private Header(byte group, byte @var, ushort start, ushort stop)
	{
		this.id = new opendnp3.GroupVariationID(group, @var);
	...
	private Header(byte group, byte @var, byte count)
	{
		this.id = new opendnp3.GroupVariationID(group, @var);
...
//ORIGINAL LINE: value.count8.value = count;
	...
	private Header(byte group, byte @var, ushort count)
	{
		this.id = new opendnp3.GroupVariationID(group, @var);
