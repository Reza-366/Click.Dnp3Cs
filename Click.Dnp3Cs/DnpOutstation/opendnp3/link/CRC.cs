...
public class CRC
{
	public static ushort CalcCrc(byte[] input, /*size_t*/int length)
	{
		ushort CRC = 0;

		for (uint i = 0; i < length; ++i)
		{
			byte index = (CRC ^ input[i]) & 0xFF;
...
//	static ushort CalcCrc(in RSeq</*size_t*/int> view);
	...
	{
		ushort crc = CRC.CalcCrc(input, new /*size_t*/int(length));

		ser4cpp.wseq_t buffer = new ser4cpp.wseq_t(input + length, 2);
		ser4cpp.Globals.Bit16<ushort, 0, 1>.write_to(buffer, crc);
	}

	public static bool IsCorrectCRC(byte input, /*size_t*/int length)
	{
