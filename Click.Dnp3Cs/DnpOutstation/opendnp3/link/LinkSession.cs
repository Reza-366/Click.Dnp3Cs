...
{
	public static LinkSession Create(in Logger logger, ulong sessionid, IResourceManager manager, IListenCallbacks callbacks, IAsyncChannel channel)
	{
...
//ORIGINAL LINE: return session;
	...
	public LinkSession(in Logger logger, ulong sessionid, IResourceManager manager, IListenCallbacks callbacks, IAsyncChannel channel)
	{
...
//ORIGINAL LINE: auto shutdown = [self = this]()
	...
	private void ShutdownImpl()
	{
		...
		this.callbacks.OnConnectionClose(new ulong(this.session_id), this.stack);
...
//ORIGINAL LINE: auto timeout = [self = this]()
	...
	private readonly ulong session_id = new ulong();
