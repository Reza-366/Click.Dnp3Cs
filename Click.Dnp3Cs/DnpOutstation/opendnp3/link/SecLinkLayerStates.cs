...
public abstract class SecStateBase
{
	...
	public abstract SecStateBase OnResetLinkStates(LinkContext UnnamedParameter, ushort source);
	public abstract SecStateBase OnRequestLinkStatus(LinkContext UnnamedParameter, ushort source);

	public abstract SecStateBase OnTestLinkStatus(LinkContext UnnamedParameter, ushort source, bool fcb);
	public abstract SecStateBase OnConfirmedUserData(LinkContext UnnamedParameter, ushort source, bool fcb, bool isBroadcast, in Message message);
...
public abstract class SLLS_TransmitWaitBase <NextState> : SecStateBase
{
	...
	public override SecStateBase OnResetLinkStates(LinkContext ctx, ushort source)
	{
	...
	public override SecStateBase OnRequestLinkStatus(LinkContext ctx, ushort source)
	{
		if (ctx.logger.is_enabled(opendnp3.flags.Globals.WARN))
		{
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
	...
	public override SecStateBase OnTestLinkStatus(LinkContext ctx, ushort source, bool fcb)
	{
		if (ctx.logger.is_enabled(opendnp3.flags.Globals.WARN))
		{
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
	...
	public override SecStateBase OnConfirmedUserData(LinkContext ctx, ushort source, bool fcb, bool isBroadcast, in Message message)
	{
		if (ctx.logger.is_enabled(opendnp3.flags.Globals.WARN))
		{
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
...
//ORIGINAL LINE: char const* Name() const override
	...
	public override SecStateBase OnResetLinkStates(LinkContext ctx, ushort source)
	{
		ctx.QueueAck(new ushort(source));
		ctx.ResetReadFCB();
	...
	public override SecStateBase OnRequestLinkStatus(LinkContext ctx, ushort source)
	{
		ctx.QueueLinkStatus(new ushort(source));
