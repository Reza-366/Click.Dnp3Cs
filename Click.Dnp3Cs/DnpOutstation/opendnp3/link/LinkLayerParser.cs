...
public class LinkLayerParser
{
	...
	public void OnRead(/*size_t*/int numBytes, IFrameSink sink)
	{
		buffer.AdvanceWrite(new /*size_t*/int(numBytes));
...
//ORIGINAL LINE: WSeq</*size_t*/int> WriteBuff() const
	public WSeq</*size_t*/int> WriteBuff()
	{
		return WSeq</*size_t*/int>(buffer.WriteBuff(), buffer.NumWriteBytes());
	}
...
//ORIGINAL LINE: return this->statistics;
	...
	private void PushFrame(IFrameSink sink)
	{
		...
		buffer.AdvanceRead(new /*size_t*/int(frameSize));
	}
	...
	private bool ValidateBody()
	{
		...
		if (LinkFrame.ValidateBodyCRC(buffer.ReadBuffer() + opendnp3.Globals.LPDU_HEADER_SIZE, new uint(len)))
		{
...
//ORIGINAL LINE: opendnp3::HexLogging::log(logger, flags::LINK_RX_HEX, buffer.ReadBuffer().take(frameSize), ' ', 10, 18);
	...
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
		...
		byte user_data_length = header.GetLength() - opendnp3.Globals.LPDU_MIN_LENGTH;
//C++ TO C# CONVERTER TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//ORIGINAL LINE: frameSize = LinkFrame::CalcFrameSize(user_data_length);
		frameSize.CopyFrom(LinkFrame.CalcFrameSize(new byte(user_data_length)));
