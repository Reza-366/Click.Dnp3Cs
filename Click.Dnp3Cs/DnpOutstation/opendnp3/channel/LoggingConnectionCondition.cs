...
public class LoggingConnectionCondition
{

	public LoggingConnectionCondition(Logger logger)
	{
	...
	public Iterator functorMethod<Iterator>(in std::error_code ec, Iterator next)
	{
		if (ec != null)
		{
			if (logger.is_enabled(opendnp3.flags.Globals.WARN))
			{
				...
				SAFE_STRING_FORMAT(message_buffer_some_name_no_conflict, opendnp3.Globals.max_log_entry_size, "connection error: %s",__VA_ARGS__);
