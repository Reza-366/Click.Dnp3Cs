...
public class ControlRelayOutputBlock
{
	...
	public ControlRelayOutputBlock(OperationType opType_ = OperationType.LATCH_ON, TripCloseCode tcc_ = TripCloseCode.NUL, bool clear_ = false, byte count_ = 1, uint onTime_ = 100, uint offTime_ = 100, CommandStatus status_ = CommandStatus.SUCCESS)
	{
...
//ORIGINAL LINE: this.offTimeMS = offTime_;
		...
		this.rawCode = ((TripCloseCodeSpec.to_type(tcc) << 6) & 0xC0) | (((byte)clear << 5) & 0x20) | (OperationTypeSpec.to_type(opType) & 0x0F);
	}

	// overloaded constructor that allows the user to set a raw control code for non-standard codes
	public ControlRelayOutputBlock(byte rawCode_, byte count_ = 1, uint onTime_ = 100, uint offTime_ = 100, CommandStatus status_ = CommandStatus.SUCCESS)
	{
