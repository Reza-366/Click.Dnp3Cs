...
public class CountIndexParser
{
	...
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
			...
			{
				return ParseCountOfObjects(buffer, record, numparser, new ushort(count), pLogger, pHandler);
			}
			else
			{
				return ParseCountOfIndices(buffer, record, numparser, new ushort(count), pLogger, pHandler);
			}
		}
...
//ORIGINAL LINE: template<class Descriptor>
	private static CountIndexParser From<Descriptor>(ushort count, in NumParser numparser)
	{
		/*size_t*/int SIZE = (/*size_t*/int)count * (Descriptor.Size() + numparser.NumBytes());
...
//ORIGINAL LINE: template<class Type>
	private static CountIndexParser FromType<Type>(ushort count, in NumParser numparser)
	{
		/*size_t*/int SIZE = (/*size_t*/int)count * (Type.Size() + numparser.NumBytes());
	...
	private static ParseResult ParseCountOfObjects(ser4cpp.rseq_t buffer, in HeaderRecord record, in NumParser numparser, ushort count, Logger pLogger, IAPDUHandler pHandler)
	{
		...
		case (GroupVariation.Group111Var0):
			return ParseIndexPrefixedOctetData(buffer, record, numparser, new ushort(count), pLogger, pHandler);
...
//ORIGINAL LINE: auto read = [&numparser, record](ser4cpp::rseq_t& buffer, uint pos)->ushort
			...
			var collection = opendnp3.Globals.CreateBufferedCollection<ushort>(buffer, new ushort(count), read);
	...
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
		...
		uint TOTAL_SIZE = count * (numparser.NumBytes() + record.variation);
	...
	//C++ TO C# CONVERTER TASK: There is no direct equivalent in C# to the following C++ macro:
		...
		if (pHandler != null)
		{
			var read = (ser4cpp.rseq_t buffer, uint pos) =>
			{
				...
				return new opendnp3.Indexed<opendnp3.OctetString>(opendnp3.Globals.WithIndex(octets, new ushort(index)));
			});

			var collection = opendnp3.Globals.CreateBufferedCollection<new Indexed<OctetString>>(buffer, new uint(count), read);
			pHandler.OnHeader(new PrefixHeader(record, count), collection);
		}
...
//ORIGINAL LINE: template<class Descriptor>
	private static void InvokeCountOf<Descriptor>(in HeaderRecord record, ushort count, in NumParser numparser, in ser4cpp.rseq_t buffer, IAPDUHandler handler)
	{
		...
		var collection = opendnp3.Globals.CreateBufferedCollection<new Indexed<typename Descriptor.Target>>(buffer, new ushort(count), read);
...
//ORIGINAL LINE: template<class Type>
	private static void InvokeCountOfType<Type>(in HeaderRecord record, ushort count, in NumParser numparser, in ser4cpp.rseq_t buffer, IAPDUHandler handler)
	{
		...
		var collection = opendnp3.Globals.CreateBufferedCollection<new Indexed<Type>>(buffer, new ushort(count), read);
		handler.OnHeader(new PrefixHeader(record, count), collection);
	}

	private CountIndexParser(ushort count, /*size_t*/int requiredSize, in NumParser numparser, HandleFun handler)
	{
