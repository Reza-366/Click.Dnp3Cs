...
//ORIGINAL LINE: this.variation = variation;
	...
	public override ushort WriteSome(IEventWriter<typename T.meas_t> writer)
	{
	...
	private bool WriteOne(IEventWriter<typename T.meas_t> writer)
	{
		...
		// unable to write
		if (!writer.Write(data.value.value, new ushort(record.index)))
		{
			return false;
		}
