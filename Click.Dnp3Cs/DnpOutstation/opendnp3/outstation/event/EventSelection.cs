...
//ORIGINAL LINE: template<class T>
	public static uint SelectByType<T>(EventLists lists, uint max)
	{
...
//C++ TO C# CONVERTER WARNING: The original C++ template specifier was replaced with a C# generic specifier, which may not produce the same behavior:
//ORIGINAL LINE: template<class T>
	public static uint SelectByType<T>(EventLists lists, T.event_variation_t variation, uint max)
	{
		return SelectByTypeGeneric<T>(lists, false, variation, ser4cpp.Globals.max);
	}

	public static uint SelectByClass(EventLists lists, in ClassField clazz, uint max)
	{
		uint num_selected = 0;
		...
		return new uint(num_selected);
	}

//C++ TO C# CONVERTER WARNING: The original C++ template specifier was replaced with a C# generic specifier, which may not produce the same behavior:
//ORIGINAL LINE: template<class T>
	private static uint SelectByTypeGeneric<T>(EventLists lists, bool useDefaultVariation, T.event_variation_t variation, uint max)
	{
