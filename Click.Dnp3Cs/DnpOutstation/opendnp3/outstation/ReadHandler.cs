...
public sealed class ReadHandler : IAPDUHandler
{
	...
	public override sealed bool IsAllowed(uint headerCount, GroupVariation gv, QualifierCode qc)
	{
		return true;
	}
...
//ORIGINAL LINE: return pEventSelector->SelectCount(header.enumeration, header.count);
		return new opendnp3.IINField(pEventSelector.SelectCount(header.enumeration, new ushort(header.count)));
	}

	private override IINField ProcessHeader(in PrefixHeader header, in ICollection<ushort> indices)
	{
//C++ TO C# CONVERTER TASK: The following line was determined to contain a copy constructor call - this should be verified and a copy constructor should be created:
