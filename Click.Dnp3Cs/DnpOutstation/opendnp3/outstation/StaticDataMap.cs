...
//	StaticDataMap() = default;
	public StaticDataMap(in SortedDictionary<ushort, typename Spec.config_t> config)
	{
...
//ORIGINAL LINE: this->selected = this->selected.Union(Range::From(start->first, stop));
		this.selected.CopyFrom(this.selected.Union(Range.From(start.first, new ushort(stop))));
